...
def main():
    ...
    while number_elem < 5000:
        ...
        print(href)  # Заходим на страницу и парсим все ссылки на полные отзывы.
        # time.sleep(random.randint(10, 25))
        for k in range(len(href)):
            sec_response = requests.get(second_url + href[k])
            ...
            names = soup.find_all(class_="lenta-card__book-title")
            texts = soup.find_all("div", {"id": "lenta-card__text-review-escaped"})
            authors = soup.find_all(
                class_="lenta-card__author")  # заходим на в каждый отзыв и берем оттуда все нужные данные
            # print(str(len(new_quotes)) + " " + str(len(names))+" "+str(len(texts))+" "+str(len(authors))) количество найденных данных
            # Используется не количество найденных ссылок а мин значение из найденых элементов, так ка бывает что отзыв без оценки или автора..
            time.sleep(random.randint(30, 45))
            y = min(len(authors), len(names), len(quotes), len(texts))
            for i in range(y):
                x = float(new_quotes[
                              i])  # Сортируем отзывы по оценкам и записываем в txt файл, W+ запись и чтение(модификатор)
                if 4.5 <= x <= 5:
                    quotes_5 = quotes_5 + 1
            ...
            # прибавляем счетчик
            # time.sleep(random.randint(30, 45))                #режим ждуна шоб не забанило
